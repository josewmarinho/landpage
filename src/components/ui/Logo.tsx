...
export function Logo({ variant = 'light', className, withTagline = true }: LogoProps) {
  const edinhoColor = variant === 'light' ? '#FDB913' : '#102A66';
  const araujoColor = variant === 'light' ? '#FFFFFF' : '#2E9E3F';
  const taglineColor = variant === 'light' ? '#C7D2EC' : '#5B6B8C';

  return (
    <span className={`inline-flex flex-col leading-none ${className ?? ''}`} aria-label="Edinho Araújo">
      <span className="flex items-center gap-0.5">
        <span className="font-display text-2xl tracking-tight sm:text-[1.7rem]" style={{ color: edinhoColor }}>
          EDINHO
        </span>
